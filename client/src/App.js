...
function App() {
  ...
  const catchLyrics = (title, artist) => {
    console.log(title, artist);
    const options = {
      apiKey: 'XPyGPImA2Ya0PLBqb-M9oQhv2Oftd_HEWhnht_FvCB9Qsr4CMzIVg3LjUdsTKwd6',
      title: title,
      artist: artist,
      optimizeQuery: true
    }

    getSongLyrics(options).then((lyrics) => console.log(lyrics));

    fetchSong(options).then((song) =>
  ...
  const handleTrackSelect = async (track) => {
    ...
    catchLyrics(track.name, track.artists[0].name);
  }
